...
S1 현재는 단일 표면이다. 아래는 콘솔 전용 표면이 생기는 시점(MS2-46 이후)에 적용한다.

API 표면에 따라 정본의 수명이 다르다. 표면이 갈리는 시점에 스펙 파일에 경로별 표면을 표기해, "이 YAML 누가 고치나" 논쟁이 생기지 않게 한다.
...
## 구현 스냅샷 (`generated/openapi.yaml`)

`generated/openapi.yaml`은 springdoc이 실행 중인 앱을 인트로스펙션해 만든 **구현 스냅샷**이다. 손으로 고치지 않는다. 고쳐도 다음 빌드에서 덮어써진다. 정본은 이 폴더의 `openapi.yaml`이며, 승격 규칙은 위 정본 정책을 따른다.

생성물을 레포에 커밋해 두는 이유는 이 파일을 읽으려는 게 아니라 **PR diff에 API 표면 변화가 드러나게 하려는 것**이다. 커밋하지 않으면 엔드포인트가 새로 열리거나 응답 스키마가 바뀌어도 리뷰어가 컨트롤러 코드를 한 줄씩 읽어야만 알 수 있다.

갱신은 개발자가 로컬에서 한다. `backend/`에서 `./gradlew build`를 돌리면 `OpenApiSnapshotTest`가 이 파일을 다시 만들고, 바뀐 내용을 소스와 **같은 커밋에** 넣는다. CI는 파일을 고치거나 커밋하지 않는다. 빌드 후 생성물이 커밋된 내용과 다르면 backend job을 실패시킬 뿐이다. `pnpm-lock.yaml`과 같은 계약이다.

CI가 봇 커밋으로 대신 갱신하지 않는 이유는 두 가지다. main 룰셋에 `dismiss_stale_reviews_on_push`가 걸려 있어 봇이 PR 브랜치에 푸시하면 기존 승인이 매번 무효화된다. 그리고 CI가 조용히 고쳐 주면 정작 작성자 본인이 자기 PR의 API 표면 변화를 못 보게 되어, 이 장치를 만든 이유가 사라진다.

`npm run lint`는 정본 `openapi.yaml`만 검사한다. 생성물은 린트 대상이 아니다. 구현이 명세를 지키는지는 린트가 아니라 두 파일의 대조로 확인한다.
