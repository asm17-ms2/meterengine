...
- 범위: 이벤트 수집 단건과 배치(배치는 명세만, 구현은 S1 제외), 미터 정의, 단가 설정, 집계 조회, draft 인보이스 조회. 경로 8개 / 오퍼레이션 10개
...
| 항목 | 결정 | 지위 |
| --- | --- | --- |
| properties 키 정책 | 스키마 검증은 안 하되 키 50개, 이름 64자 상한. 값은 문자열/숫자/불리언/null. 중첩은 저장하되 집계 대상 아님 | ERD가 이관 |
