...
| 금액 타입 | 원 단위 정수 | `DraftInvoiceResponse.DraftInvoiceLine.amount`, `openapi.yaml`의 `DraftInvoiceLine` | 초기 관례 |
...
| 이벤트가 없는 고객 | 단가가 있는 모든 미터 라인을 수량 0, 금액 0으로 갖는다 | `DraftInvoiceService.BillableMetricQuantitiesByCustomer.toDraftInvoiceLine`, `DraftInvoiceIntegrationTest.이벤트가_없는_고객도_금액_0으로_응답에_들어간다` | 초기 관례 |
