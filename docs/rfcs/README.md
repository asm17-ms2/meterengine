...
되돌리면 외부 계약, 이미 쌓인 데이터, 또는 그 위에 선 코드 전체(프레임워크, 라이브러리, 저장소, 레이어 규칙)가 같이 움직이는 결정에만 씁니다 (RFC-003).
예) 이벤트 스키마의 모양, 오류 응답의 모양, JPA 대신 jOOQ.

결정 하나에 한 장입니다. 따로 바꿀 수 있는 항목은 규칙이므로 RFC에 쓰지 않습니다.
팀 규칙(코드, 문서, 브랜치, PR을 쓰는 법)은 `CONTRIBUTING.md`와 `docs/contributing/`에, 인보이스 확정 대기 기간 같은 값은 `docs/policies/`에 씁니다.
가르는 기준과 머지된 RFC를 고치는 규칙은 `docs/contributing/governance.md`가 정본입니다.
애매하면 규칙이나 정책 PR로 올리고, 리뷰에서 "RFC급" 지적이 나오면 RFC로 승격합니다.
...
도메인은 frontmatter `domain`과 같은 값이다. process거나, 백엔드 최상위 패키지명과 같은 이름(`customer`, `invoice`, `global` 등)을 쓴다. 그 결정을 바꾸면 무너지는 도메인을 전부 적고, 순서에 뜻을 두지 않는다. 정책 파일 이름도 이를 따른다.
