...
인보이스 확정 대기 기간 같은 세부 규칙은 여기가 아니라 `docs/policies/`에, 코드와 문서와 PR을 쓰는 방법은 `docs/contributing/`에 씁니다. 층을 가르는 기준은 `docs/contributing/decisions.md`에 있습니다.
...
결정을 뒤집을 때는 옛 파일을 지우지 않고 새 RFC를 씁니다. 새 문서에는 어느 결정을 대체하는지를(frontmatter `supersedes`), 옛 문서에는 대체됐다는 상태(`status: superseded`)와 어느 결정으로 대체됐는지를(`superseded-by`) 적습니다. 옛 문서는 새 RFC의 PR에서 같이 고칩니다. 머지된 RFC의 본문은 그 외에는 고치지 않습니다. 결정의 일부만 바뀔 때의 표시와, 새 RFC 없이 규칙 파일만 고치는 경우는 `docs/contributing/decisions.md` "규칙 개정"에 있습니다.
...
| [005](005-rule-management.md) | RFC는 방향 결정 한 장으로 좁히고 팀 규칙은 살아 있는 문서로 둔다 | process | draft | 2026-09-04 |
