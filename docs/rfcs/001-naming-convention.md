...
이 RFC는 제안서와 PR의 방향을 하나로 합치고, 제안서가 다루지 않은 항목(컬렉션, 테스트 이름, 파일과 패키지, 프론트)을 채운다. 기준은 하나다. **외부 표준 관례를 따르고, 벗어날 때는 이유를 적는다.** 대조한 근거는 Google Java/TypeScript Style Guide, JLS §6.1, JavaBeans 명세, JDK와 Spring의 실제 API 이름, Google API 설계 가이드(AIP), Zalando API 가이드, Stripe API, React와 Next.js 공식 문서, Vercel 공식 예제(next-learn), Airbnb 가이드, typescript-eslint `naming-convention` 규칙의 옵션 기본값이다. 근거의 종류는 규칙 파일의 근거 열에 있고, 이 문서의 링크는 문서 끝에 모았다. 표준이 없는 항목은 이름을 고를 자유도가 가장 작은 쪽을 택했다.
