...
| USAGE_EVENT.received_at | BEFORE INSERT 트리거가 항상 DB 서버 시각(`clock_timestamp()`)으로 덮어쓴다 -- 앱이 값을 넣어도 무시되어 "클라이언트 입력 불가"가 DB로 강제된다 (DEFAULT는 안전망으로 유지). 재검토 조건: 수집 경로가 비동기(큐 도입 등)로 바뀌면 수신 시각의 의미(앱 수신 vs DB 저장)를 재정의한다 | 결정 2, PR #13 리뷰 |
...
| v2.2 | 2026-07-30 | PR #13 리뷰 반영 2차: received_at을 BEFORE INSERT 트리거로 항상 DB 서버 시각 덮어쓰기(수집 비동기화 시 재검토 조건 기록), UPDATE 가드를 나열식에서 행 전체 비교로 변경(신규 컬럼 자동 보호) |
