...
| 식별자 | 케이스 | 형식 | 예 | 근거 |
| --- | --- | --- | --- | --- |
| 패키지 | 소문자, 밑줄 없음 | `com.meterengine.<domain>.<layer>`, domain은 단수 한 단어, layer는 `config` `controller` `dto` `entity` `repository` `service`. 한 도메인만 쓰는 설정은 `<domain>.config`. 도메인 없는 공용 코드는 `global.<역할>`이고 도메인 없는 설정은 `global.config`. 최상위 루트에는 진입점만 | `com.meterengine.invoice.dto`, `com.meterengine.payment.config.TossPaymentsProperties`, `com.meterengine.global.config.OpenApiConfig` | 스타일 가이드: [Google Java 5.2.1][gj-5.2.1](소문자). 관례: 루트에 진입점만 두는 것은 [Spring Boot 레퍼런스 코드 구조][sb-structure]의 예제 배치와 [PetClinic][petclinic]. 도메인 없는 코드를 한 패키지에 모으는 것은 PetClinic의 `system`. 이름 `global`과 단복수는 RFC-001. `config` layer와 도메인 설정의 자리는 이 파일 |
...
| 리포지토리 | UpperCamelCase | `<대상>Repository`. 대상은 엔티티, 또는 엔티티가 없는 테이블의 읽기 모델 | `InvoiceLineRepository`, `BillableMetricUsageRepository` | 관례: [Spring Data JPA 레퍼런스][sd-repos] `PersonRepository`, [Spring 가이드][guide-jpa] `CustomerRepository`, [PetClinic][petclinic] `OwnerRepository`. 엔티티 없는 대상은 이 파일 |
...
| 리포지토리 메서드 | lowerCamelCase | 파생 쿼리는 `find` `exists` `count` `delete` + `By...`만(Spring Data는 `find`와 `get`을 동의어로 보므로 `find`만 씀). 정렬 키 셋 이상이나 조건 셋 이상이면 `@Query`, JPQL이 이름과 어긋나면 서술형 이름. 손으로 짠 SQL 메서드(`JdbcTemplate`)는 파생 쿼리와 같은 동사(`find` `exists` `count` `insert` `sum`)로 시작하고 `By` 대신 서술형을 허용한다 | `findByOrganizationIdOrderByNameAscIdAsc`, `findPage`, `existsForCustomer` | 도구 동작: [Spring Data][sd-query]가 접두사를 파싱해 쿼리를 만들고 `find` `read` `get` `query` `search` `stream`을 동의어로 봄. `@Query` 전환 기준은 RFC-001, 정렬 키 문턱과 손으로 짠 메서드는 이 파일 |
...
| 정적 팩터리, 변환 메서드 | lowerCamelCase | 한 인자 변환 `from`, 여러 인자 조립 `of`, 새 객체로 변환 `toX`, 뷰 `asX`. 값의 상태를 고정하는 팩터리는 그 상태를 말하는 낱말 | `CustomerResponse.from(customer)`, `IngestEventResponse.stored(transactionId)` | 관례: JDK [`LocalDate.of`][jdk-localdate], `Instant.from`, [`List.of`][jdk-list], `Collectors.toList`, `Arrays.asList`. 상태 팩터리는 Spring [`ResponseEntity.ok()`][sf-response-entity] `noContent()`, JDK [`Optional.empty()`][jdk-optional] |

#### 필드와 변수

| 식별자 | 케이스 | 형식 | 예 | 근거 |
| --- | --- | --- | --- | --- |
| 주입 필드 | lowerCamelCase | 타입명 그대로, 같은 타입 둘 이상일 때만 수식어. 테스트의 `MockMvcTester`는 `mvc` | `customerRepository`, `mvc` | 관례: [Spring Framework 레퍼런스][sf-di] DI 예제 `private final MovieFinder movieFinder`. [PetClinic][petclinic]은 `owners`와 `vetRepository`가 섞임. `mvc`는 [Spring Boot 레퍼런스 모의 환경 테스트][sb-mock-mvc] 예제 `private MockMvcTester mvc`([Spring Framework 레퍼런스 MockMvc AssertJ][sf-mockmvc-assertj]는 `mockMvc`, 타입명을 쓰는 예제는 없음). 적용은 [2f48aea][l-inject] |
...
| 그 외 변수, 파라미터 | lowerCamelCase | 값의 정체를 말하는 명사구, 조회 메서드 이름과 맞춤. Map은 컬렉션 행이 먼저 | `billableMetricUsage`, `targetMonth` | 스타일 가이드: [Google Java 5.2.7][gj-5.2.7](케이스). 명사구와 조회 메서드 일치는 [ea7abd2][l-nounphrase]. 그 커밋의 `baseUnitPrices`는 Map이라 지금은 컬렉션 행을 따른다 |
| boolean | lowerCamelCase | 필드, 변수, record 컴포넌트, JSON 키, DB 컬럼 전부 접두사 없는 형용사나 과거분사로 같은 이름. 접두사 `is` `has` `can`은 접근자 메서드에만. 접두사 없는 이름이 자바 키워드면 접근자와 같은 이름을 필드명으로 쓴다 | 필드 `finalized`, 접근자 `isFinalized()`, JSON과 컬럼 `finalized`, 키워드 예외 필드 `isNew`와 접근자 `isNew()` | 명세: [JavaBeans][javabeans] 8.3.2(접근자 `isX()`). 스타일 가이드: [Google Java][gj]는 필드 접두사 규칙 없음, JSON 접두사 생략은 [AIP-140][aip-140]. 한 값에 이름 하나를 두어 `@Column`과 `@JsonProperty`로 이름을 맞출 일을 없애는 것은 RFC-001. 관례: 키워드 예외는 [Spring Data JPA Persistable 예제][sd-is-new]의 `private boolean isNew = true`와 `isNew()` |
...
| 테스트 위치 | | 단위 테스트(`<클래스>Test`)는 대상과 같은 패키지. 관점이 붙는 테스트는 도메인 루트. 도메인에 속하지 않는 것은 `global`이며, 공용 클래스의 단위 테스트는 그 클래스와 같은 `global.<역할>`, 산출물 테스트(시드, OpenAPI 문서, 스키마 제약)와 관점이 붙는 테스트는 `global` 바로 아래. 루트에는 애플리케이션 테스트, 그리고 Spring Boot 레퍼런스가 정한 개발 시 진입점(`TestMeterEngineApplication`)과 그 컨테이너 설정(`TestcontainersConfiguration`)만. 이 둘은 테스트가 아니라 진입점이라 생성된 이름과 자리 그대로다. 메인 코드의 루트에 진입점만 두는 것과 같은 기준 | `customer/service/CustomerServiceTest`, `invoice/InvoiceRoundTripTest`, `global/SeedDataTest` | 관례: 대상과 같은 패키지는 [PetClinic][petclinic]. 관점이 붙는 테스트를 도메인 루트에 둔 것은 [4945e92][l-test-loc]. `global`은 패키지 행. 개발 시 진입점과 컨테이너 설정의 자리와 이름은 [Spring Boot 레퍼런스 개발 시 Testcontainers][sb-dev-tc] 예제 `src/test/java/com/example/TestMyApplication.java`, `MyContainersConfiguration` |
...
| 식별자 | 케이스 | 형식 | 예 | 근거 |
| --- | --- | --- | --- | --- |
| URL 경로 | kebab-case | 복수 명사, 동사 없음. 부모 아래 하나뿐인 하위 리소스는 단수. 셀 수 없는 명사는 단수 그대로 | `/v1/billable-metric-prices`, `/v1/billable-metrics/{code}/price-policy`, `/v1/usage` | 스타일 가이드: [Zalando 규칙 129][zalando-129], [134][zalando-134], [141][zalando-141]. [AIP-122][aip-122], 단수 하위 리소스는 [AIP-156][aip-156]. 셀 수 없는 명사는 프론트 컴포넌트 행과 같은 기준으로 이 파일 |
...
| JSON 키 | snake_case | 두 낱말 이상이면 `@JsonProperty`로 지정하고 한 낱말에는 붙이지 않는다. boolean은 접두사 없는 형용사 | `customer_id`, `active`, 한 낱말 `name`은 어노테이션 없음 | 스타일 가이드: [Zalando 규칙 118][zalando-118], boolean 접두사 생략은 [AIP-140][aip-140]. 관례: [Stripe][stripe]. Google REST와 Azure는 camelCase라 Stripe/Zalando 계열을 택함. 한 낱말 생략은 이 파일 |
...
[sd-is-new]: https://docs.spring.io/spring-data/jpa/reference/jpa/entity-persistence.html#jpa.entity-persistence.saving-entities.strategies
...
[sb-dev-tc]: https://docs.spring.io/spring-boot/reference/features/dev-services.html#features.dev-services.testcontainers.at-development-time
[sb-mock-mvc]: https://docs.spring.io/spring-boot/reference/testing/spring-boot-applications.html#testing.spring-boot-applications.with-mock-environment
[sf-response-entity]: https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/http/ResponseEntity.html
[sf-mockmvc-assertj]: https://docs.spring.io/spring-framework/reference/testing/mockmvc/assertj/setup.html
...
[jdk-optional]: https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/Optional.html
