...
오류 응답의 모양은 RFC-004가 정했다. 이 파일은 그 모양을 채울 때 팀원이 따르는 규칙이다. 서버 안 구조(예외 계층, 핸들러 배치)는 코드가 정본이고 이 파일은 자리만 정한다. 이름은 `naming.md` "오류 처리" 절이다.
...
- 오류 응답의 모양은 통합 테스트가 실제 응답 본문을 받아 검증한다. 생성된 OpenAPI 문서는 애너테이션에서 나오므로 실제 응답과 다를 수 있고, 문서 검사만으로는 그 어긋남을 잡지 못한다.
...
- RFC-004가 정한 오류 응답의 모양은 되돌리면 도입사와 프론트엔드 계약이 같이 움직이지만, 이 파일의 규칙은 행 하나를 고치면 되돌아간다. governance.md의 기준대로 둘을 가른다.
