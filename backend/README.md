...
- API 명세: spec-first로 시작한다. 명세는 `docs/api/`에 두고(MS2-27에서 작성 중), springdoc-openapi는 구현 스냅샷을 만들어 명세와 대조하는 용도로 쓴다. 정본 정책은 API 표면별로 다르다: 고객 대면 API는 수기 명세가 계속 정본, 콘솔 내부 API는 구현 완료 후 springdoc 생성물로 정본을 승격한다 (상세는 `docs/api/README.md`)
