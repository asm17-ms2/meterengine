...
- PostgreSQL 단일 저장소 (ADR 0002). DB 접근은 기본 Spring Data JPA, 단 append-only 원장(usage_events)과 집계 SQL은 JdbcClient를 쓴다 (ADR 0006). 집계는 사전 집계 없이 SQL로 계산한다 (ADR 0004)
...
### 이벤트 수집 API 로컬 확인 (MS2-38 임시)

인증 스텁이 들어오기 전까지 수집 이벤트는 `application.properties`의 `meterengine.ingestion.organization-id`에 설정된 고정 도입사로 귀속된다. 로컬에서 API를 호출하려면 그 id로 도입사 행을 한 번 시드해야 한다:

```
docker exec meterengine-postgres-1 psql -U meterengine -c \
  "INSERT INTO organizations (id) VALUES ('00000000-0000-0000-0000-000000000001') ON CONFLICT DO NOTHING;"
```
