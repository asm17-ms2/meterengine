...
고객 삭제에서 나는 FK 위반은 advice까지 가지 않는다. `CustomerService.delete`가 제약 이름으로 `invoice_customer_same_org`와 나머지를 갈라 `CustomerHasInvoicesException`이나 `CustomerHasEventsException`으로 바꿔 던져 409가 된다. 그 자리가 뜻을 아는 유일한 곳이라서다.
