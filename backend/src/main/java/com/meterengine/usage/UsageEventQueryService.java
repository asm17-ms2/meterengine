...
class UsageEventQueryService {
   ...
   *
   * <p><b>기간 계산을 여기서 하지 않고 집계와 공유한다.</b> {@link UsageAggregationService#BILLING_ZONE}과 반열린 구간
  ...
  EventPageResponse query(
    ...
    OffsetDateTime start =
        month.atDay(1).atStartOfDay(UsageAggregationService.BILLING_ZONE).toOffsetDateTime();
    OffsetDateTime end =
        month
            .plusMonths(1)
            .atDay(1)
            .atStartOfDay(UsageAggregationService.BILLING_ZONE)
            .toOffsetDateTime();
