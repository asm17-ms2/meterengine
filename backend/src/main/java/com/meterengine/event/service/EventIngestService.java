...
public class EventIngestService {
  ...
  EventIngestService(
      EventRepository usageEvents, CustomerRepository customers, JsonMapper jsonMapper) {
    this.usageEvents = usageEvents;
    this.customers = customers;
    this.jsonMapper = jsonMapper;
  }

  public EventIngestResponse ingest(UUID organizationId, EventIngestRequest request) {
    ...
    try {
      int inserted =
          usageEvents.insertIfAbsent(
              ...
              request.timestamp());
      return inserted == 1
          ? EventIngestResponse.stored(request.transactionId())
          : EventIngestResponse.alreadyStored(request.transactionId());
      ...
      return EventIngestResponse.alreadyStored(request.transactionId());
    }
  }
}
