...
class CustomerCrudIntegrationTest {
  ...
  /**
   * customer를 참조하는 FK가 늘면 삭제 오류의 갈래가 틀려진다.
   *
   * <p>{@code CustomerService.delete}는 FK 위반을 {@code invoice_customer_same_org}와 나머지로 갈라 409 code를
   * 고른다. 셋째 FK가 생기면 그 위반이 "이벤트가 있다"로 나가므로, 여기서 먼저 깨져 갈래를 다시 정하게 한다. 제약 이름을 마이그레이션에서 바꿀 때도 같다.
   */
  @Test
