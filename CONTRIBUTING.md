...
- main 상태: main은 항상 빌드/테스트가 통과하는 상태를 유지한다. 깨진 코드나 반쯤 만든 기능은 브랜치에만 둔다. CI 실패 시 머지를 금지한다. 필수 체크 항목은 CI 워크플로(`.github/workflows/ci.yml`)의 `backend`, `frontend`, `docs` 세 job이다 (MS2-31에서 지정). main 룰셋의 Required status checks에 두 job을 등록해 강제한다
